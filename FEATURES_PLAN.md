...
2. **Không có permission engine** ([ADR 0003](apps/backend/docs/project/decisions/0003-no-permission-engine.md)): không decorator, không bảng role/permission. Service tự `if/else` dùng helper `isAppAdmin(user, app, orgId)`.
...
| --- | --- | --- | --- | --- | --- |
| 0 | **Foundation** | core, common | Migrate `src/modules/` → `src/apps/core/`, `BaseRepository`, `audit_logs` table + `@Auditable` interceptor, `isAppAdmin` helper, fix lint FE | Fix 2 lỗi lint pre-existing, sidebar nav slot cho HRM/Attendance/Requests (disabled cho đến khi feature implement) | — |
| 1 | **Auth + Org + User + AppAdmin** | core, platform, hrm | `Organization`, mở rộng `User` (`userType`, `title`, `organizationId`, `employeeId`), `AppAdmin` model + module gán/xoá, signup flow Org | Trang đăng ký Org (tạo Org + admin user), trang `/settings/organization`, trang `/settings/app-admins` (HRM appadmin grant/revoke), profile edit (set `User.title`) | 0 |
...
- [ ] **`isAppAdmin` helper** trong `src/common/auth/access.ts` (xem [ADR 0003](apps/backend/docs/project/decisions/0003-no-permission-engine.md)) — chưa enforce ở Feature 0, chuẩn bị cho Feature 1.
...
Chuyển app từ "1 hệ thống User" → "Org multi-tenant với 2 loại user (SYS_OWNER + ORG_USER) + AppAdmin per-app".

### BE

- [ ] Prisma:
  ```prisma
  enum UserType { SYS_OWNER ORG_USER }
  ...
  model User {
    ...
    userType       UserType  @default(ORG_USER)                    @map("user_type")
  ...
  ```
- [ ] Migration: drop `enum UserRole` cũ (ADMIN/USER) → seed mới chuyển `ADMIN` → `SYS_OWNER`, `USER` → `ORG_USER` (cần `organization_id` nullable trong migration step 1).
- [ ] **Module `core/organization`**:
  - `POST /api/v1/organizations/signup` — public, tạo Org + ORG_USER đầu tiên + auto-grant HRM AppAdmin trong 1 transaction.
  - `GET /api/v1/organizations/me` — Org của user đang login.
  - `PATCH /api/v1/organizations/me` — chỉ HRM appadmin (hoặc tách Org settings ra Setting AppAdmin riêng — defer).
- [ ] **Module `core/user`** (mở rộng):
  - `GET /api/v1/users/me` — trả `{ user, organization, appAdmins[] }`.
  - `PATCH /api/v1/users/me` — đổi `name`, `title`, `avatar`. Không đổi `userType`/`organizationId`/`employeeId`.
  - `PATCH /api/v1/users/me/password`.
- [ ] **Module `hrm/app-admin`**:
  - `GET /api/v1/app-admins?app=HRM` — list HRM appadmins của Org.
  - `POST /api/v1/app-admins` body `{ userId, appCode }` — gán (chỉ HRM appadmin của cùng Org gán được HRM appadmin khác; SYS_OWNER override). `@Auditable`.
  - `DELETE /api/v1/app-admins/:id` — xóa. `@Auditable`.
- [ ] `JwtStrategy` payload mở rộng: `{ userId, userType, organizationId, employeeId }`. Set vào `RequestContextService`.
- [ ] Update seed: tạo 1 SYS_OWNER + 1 Org `acme-demo` + 1 ORG_USER admin + auto-grant HRM appadmin.
...
- [ ] `useIsAppAdmin(app: AppCode)` hook — check từ `me.appAdmins`.
- [ ] `<DashboardShell>`:
  - Header hiển thị `organization.name` + `user.name` + `user.title` (nếu có) + dropdown logout.
  - Sidebar: nav HRM/Settings ẩn cho user không phải HRM appadmin (read-only nav nếu cần).
- [ ] `features/settings/`:
  - `/settings/account` — edit `User.title`, `name`, `avatar`, đổi password.
  - `/settings/organization` — edit Org `name`, `timezone`, `currency`. Chỉ HRM appadmin thấy.
  - `/settings/app-admins` — table list + button "Add appadmin" (modal autocomplete user), button remove. Chỉ HRM appadmin thấy.

### Done-when

- 2 Org độc lập tạo qua UI; login chéo trả 403.
- SYS_OWNER seed thấy được mọi Org qua tool internal (chưa cần UI).
- HRM appadmin của Org A grant HRM appadmin cho user khác trong Org A; không grant được cho user Org B.
...
- [ ] **Module `hrm/orgchart`**:
  ...
  - Cache Redis `orgchart:chain:${employeeId}` TTL 1h. Invalidate qua EventEmitter khi `Employee.departmentId`, `Department.managerId`, `Department.parentDepartmentId` đổi.
...
- [ ] `features/orgchart/`:
  ...
  - 2 view tab switch:
    ...
    - **Department structure**: vẽ theo `Department.parentDepartmentId`, mỗi node = Department + manager portrait + employee count.
...
- [ ] Prisma:
  ```prisma
  enum WorkScheduleType { FIXED SHIFT }
...
- [ ] **Module `attendance/work-schedule`**:
  - CRUD `/api/v1/work-schedules`. Chỉ HRM appadmin.
...
- [ ] **Module `attendance/timesheet`**:
  - `GET /api/v1/timesheet?employeeId=&year=&month=` — return:
    ...
    {
      "year": 2026, "month": 5,
      "workSchedule": { "workingDays": [...], "startTime": "08:00", "endTime": "17:00", "lateGraceMinutes": 15 },
      "days": [
        { "date": "2026-05-01", "checkInAt": null, "checkOutAt": null, "isWorkingDay": true, "status": "ABSENT" },
        { "date": "2026-05-02", "checkInAt": "08:12", "checkOutAt": "17:30", "isWorkingDay": true, "status": "LATE" },
        ...
      ]
    }
    ```
  - `status` derive từ logic compare log + schedule (`PRESENT | LATE | EARLY_LEAVE | ABSENT | WEEKEND`).
...
- [ ] `features/work-schedule/` — `/settings/work-schedule`. Form config Org schedule. Chỉ HRM appadmin.
...
- [ ] `features/timesheet/`:
  - `/timesheet` — calendar tháng theo ảnh user gửi:
    ...
    - Workday cells tô background theo `WorkSchedule.workingDays`; weekend trắng.
