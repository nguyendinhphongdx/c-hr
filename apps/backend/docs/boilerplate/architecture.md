...
The single place where every module is wired. New feature modules **must** be registered here. See [src/app.module.ts](../../src/app.module.ts).
...
`JwtAuthGuard` already populates `userId` and `sessionId`. Add new context keys by extending `RequestContextData` in [src/common/context/request-context.service.ts](../../src/common/context/request-context.service.ts).
