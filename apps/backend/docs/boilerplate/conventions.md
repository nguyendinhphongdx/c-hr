...
- Cross-module shared → [`src/common/types/pagination.dto.ts`](../../src/common/types/pagination.dto.ts) + [`src/common/types/pagination.types.ts`](../../src/common/types/pagination.types.ts)
...
2. Add the export to [src/config/index.ts](../../src/config/index.ts) and to the `configs` array.
...
`AccessException` codes are in [src/common/exceptions/access.exception.ts](../../src/common/exceptions/access.exception.ts). Add new codes there, not in random services.
