...
All cookie behavior is driven by env vars in [src/config/auth.config.ts](../../../src/config/auth.config.ts):
...
CORS credentials are already enabled in [src/main.ts](../../../src/main.ts).
...
1. Extend `RequestContextData` in [src/common/context/request-context.service.ts](../../../src/common/context/request-context.service.ts).
