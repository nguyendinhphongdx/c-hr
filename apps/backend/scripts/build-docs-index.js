 ...
 *
 * Each entry contains: id, path, title, description, tags, category, headings, summary.
...
function extractFirstParagraph(body) {
  // Skip leading headings/blank lines, return first non-empty paragraph (joined).
 ...
 *   - 'root'        → CLAUDE.md, AGENTS.md (repo root)
 *   - 'boilerplate' → docs/boilerplate/** — framework patterns, ships with the boilerplate
 *   - 'project'     → docs/project/**     — project-specific, owned by the consumer
 *   - 'overview'    → docs/README.md and any docs/* not under boilerplate/ or project/
 */
...
function categoryFromPath(relPath) {
  ...
  // docs/{boilerplate|project}/<file>.md → use the scope name (e.g. 'boilerplate')
  if (parts.length === 3) return parts[1];
  // docs/boilerplate/recipes/x.md → 'recipe'
  // docs/project/decisions/x.md   → 'decision'
...
function main() {
  ...
  // Everything under docs/ except index.json itself
  ...
  // Stable order: root → overview → boilerplate (arch/conv/recipe/reference) → project
