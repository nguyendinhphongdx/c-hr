...
export function buildTimesheetReportXlsx(args: BuildArgs): Buffer {
  ...
  const totalsRow: (string | number)[] = [
    ...
    fmtHm(rows.reduce((s, r) => s + r.otMinutesWeekday, 0)),
    fmtHm(rows.reduce((s, r) => s + r.otMinutesWeekend, 0)),
    fmtHm(rows.reduce((s, r) => s + r.otMinutesHoliday, 0)),
    fmtHm(rows.reduce((s, r) => s + r.otMinutesTotal, 0)),
