...
The factory in [src/libs/storage/storage.module.ts](../../../apps/backend/src/libs/storage/storage.module.ts) imports the right provider at startup. Trying to use S3 without installing the SDK throws a clear error.
...
In `local` mode, [src/main.ts](../../../apps/backend/src/main.ts) registers a static handler at `/uploads/*` pointing to `./uploads`. Public file URLs end up like:
