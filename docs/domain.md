...
Outline cho HRM domain (entity, invariant, business rule). Iterate qua từng feature theo [plans/refactor.md](plans/refactor.md) → [plans/features.md](plans/features.md). Mỗi quyết định lớn ghi ADR trong [decisions/](decisions/).

> UX / route map / persona phía FE đặt riêng ở [frontend/domain.md](frontend/domain.md) — file này tập trung vào BE business model.
...
> Chỉ liệt kê các entity sẽ implement. Chi tiết schema viết trong feature tương ứng (xem [plans/features.md](plans/features.md)).
