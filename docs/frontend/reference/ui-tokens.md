...
Defined in [src/app/globals.css](../../../apps/frontend/src/app/globals.css).
...
Use [`<Reveal>`](../../../apps/frontend/src/features/landing/components/Reveal.tsx) to animate content **only when scrolled into view** (instead of all at once on page load). It's a thin `IntersectionObserver` wrapper that adds `animate-fade-up` when the element enters the viewport.
